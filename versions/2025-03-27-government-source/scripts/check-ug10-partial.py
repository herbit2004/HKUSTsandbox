...
ap=argparse.ArgumentParser();ap.add_argument('--project',type=Path,default=Path(__file__).resolve().parents[1]);args=ap.parse_args();P=args.project;D=P/'public/models/hires/partial-ug10';patch=json.loads((D/'manifest.json').read_text())['patches'][0];tiles=patch['levels']['high']['tiles'];assert len(tiles)==37 and patch['partial']and not patch['evidence']['completeBranch'];base={t['id']:t for t in json.loads((P/'public/models/render-manifest.json').read_text())['tiles']};source_data=json.loads((D/'branch-audit.json').read_text());assert sum(len(t['missingDirectSiblings'])for t in source_data['subtrees'])==4
...
rng=np.random.default_rng(20260906);flat=np.unique(np.r_[np.flatnonzero(image.ravel()==0),rng.choice(image.size,12000,replace=False)]);rr,cc=np.unravel_index(flat,image.shape);xz=np.c_[m['minX']+(cc+.5)*.5,m['minZ']+(rr+.5)*.5];cross=np.cross(tri[:,1]-tri[:,0],tri[:,2]-tri[:,0]);tree=STRtree(polygons(tri[abs(cross[:,1])>1e-9][:,:,[0,2]]));pairs=tree.query(points(xz),predicate='intersects');expected=np.zeros(len(flat),bool);expected[pairs[0]]=True;assert np.array_equal(expected,image[rr,cc]>0)
