...
parser.add_argument('--render-only', action='store_true', help='Regenerate Markdown from existing acceptance JSON without changing inventory or decisions')
args = parser.parse_args()
if args.render_only:
    rows = report['rows']
    physical_groups = report.get('physicalGroupRows', [])
    bundle_count = report.get('workingTreeNativeExteriorGroups', 0)
else:
...
lines = ['# 全校逐栋实际视觉验收状态', '',
         ...
         f"已发布快照：`{report['lastPublishedSnapshot']}`。工作区{bundle_count}原生组/{report['workingTreeNativeSourceObjects']}源对象，发布与真实运行验收分别记录于QA-v4。", '',
         '| 实体 | 状态 | 已安装可验模型 |', '|---|---|---|']
