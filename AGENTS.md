...
- 用户于 2026-09-08 要求停掉本地相关服务，以后默认通过 https://hkust-sandbox.herbit2004.chatgpt.site/ 查看效果；不要保持本地预览常驻或自动重启。仅在开发验证确有需要时临时启动本地服务，完成后关闭。临时本地预览仍使用 4317，检查监听与实际目录后用 scripts/preview-update.py 原子切换；线上检查复用已有 Site 标签。
