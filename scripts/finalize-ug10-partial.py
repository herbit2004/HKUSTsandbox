...
ap=argparse.ArgumentParser();ap.add_argument('--project',type=Path,default=Path(__file__).resolve().parents[1]);args=ap.parse_args();P=args.project;D=P/'public/models/hires/partial-ug10';m=json.loads((D/'manifest-pending-floater-qa.json').read_text());patch=m['patches'][0];evidence=json.loads((D/'fine-fragment-correction-evidence.json').read_text());assert evidence['status']=='confirmed-source-floater'and evidence['combinedRemovedTriangleCount']==929 and evidence['distanceLowerBoundToAnyOtherNewTriangleMeters']>20
helper=P/'public/models/source-corrections/ug10-isolated-photogrammetry-fragment-9/build_correction.py';code=helper.read_text().replace('assert removed==9','assert removed==len(REMOVE_GLOBAL)').replace("'ug10-isolated-photogrammetry-fragment-9'","'ug10-refined-source-isolated-fragment'");namespace={'__name__':'source_correction_helper'};exec(compile(code,str(helper),'exec'),namespace)
validations=[]
for correction in evidence['components']:
 ...
 t.update(url='partial-ug10/'+dest.name,bytes=result['bytes'],sha256=result['sha256'],triangles=result['triangles'],vertices=t['vertices']-result['verticesRemoved'],bounds={'min':new_tri.min((0,1)).tolist(),'max':new_tri.max((0,1)).tolist()});t['center']=((new_tri.min((0,1))+new_tri.max((0,1)))/2).tolist();t['sourceCorrection']={'id':'ug10-refined-source-isolated-fragment','originalUrl':original['url'],'originalSha256':original['sha256'],'originalTriangles':original['triangles'],'removedTriangleIndices':correction['triangleIndices'],'evidence':'partial-ug10/fine-fragment-correction-evidence.json'};validations.append({'id':t['id'],'removedTriangles':result['trianglesRemoved'],'removedVertices':result['verticesRemoved'],'retainedPositionsExact':True,'sourceSha256':original['sha256'],'derivedSha256':result['sha256']})
tris=[]
