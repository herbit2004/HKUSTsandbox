...
async function pool(items,run,concurrency=4){let index=0;await Promise.all(Array.from({length:concurrency},async()=>{while(index<items.length){const item=items[index++];await run(item);}}));}
...
const needed=await missing();let completed=0,bytesUploaded=0;
...
async function upload(sha){
 ...
 if(Date.now()-lastLog>15000||completed===needed.length){console.log('Uploaded '+completed+'/'+needed.length+' objects; '+Math.round(bytesUploaded/1024/1024)+' MiB');lastLog=Date.now();}
}
if(needed.length)await upload(needed.shift());
await pool(needed,upload,5);
